...
lambda = 1550e-9;            % wavelength (1550 nm)
f0 = c_c/lambda;             % characteristic frequency (Bragg frequency, Hz)
omega_b = 2 * pi * f0;       % Bragg frequency (rad/s)
...
% Define detuning (user-specified for now)
delta_f = 1e9;               % Detuning frequency in Hz (e.g., 1 GHz)
delta_omega = 2 * pi * delta_f;  % Detuning in rad/s
...
RL = 0;                      % Reflected wave left (set to 0 for DFB)
RR = 0;                      % Reflected wave right (set to 0 for DFB)

Nz = 101;                     % number of z units in waveguide
dz = L/(Nz-1);               % cm unit of length
dt = dz/vg;                  % s unit of time
fsync = dt*vg/dz;            % Hz Synchronous frequency (should be 1)
Nt = floor(100*Nz);          % number of time units
...
% DFB Grating
kappa_0 = 100;               % Coupling coefficient
Lambda = lambda / (2*n_g); % grating period 
kappa = kappa_0 * cos(2*pi*z/Lambda*(pi/2));  % Sinusoidal grating
...
xlabel('time(ps)')
ylabel('Nave')
title('Average Electron Density')
hold off
...
xlabel('time(ps)')
ylabel('E')
title('Optical Output (Time Domain)')
...
for i = 2:Nt
    ...
    beta = 1i * beta_i;                 % beta = i * (gain_z - alpha)/2

    %exponential terms with detuning
    exp_det_f = exp(-1i * (delta_omega * dt + dz * beta));  % Forward: -i(delta_omega*dt + beta*dz)
    exp_det_r = exp(1i * (delta_omega * dt - dz * beta));  % Reverse: i(delta_omega*dt - beta*dz)
    ...
    % DFB update with grating coupling and detuning
    Ef(2:Nz) = fsync * exp_det_f(1:Nz-1) .* Efp(1:Nz-1) + 1i * dz * kappa(1:Nz-1) .* Er(1:Nz-1);
    Er(1:Nz-1) = fsync * exp_det_r(2:Nz) .* Er(2:Nz) + 1i * dz * kappa(2:Nz) .* Efp(2:Nz);
    ...
    S_f = abs(Ef).^2 * EtoP;  % Forward photon density 
    S_r = abs(Er).^2 * EtoP;  % Reverse photon density 
    ...
    if mod(i,plotN) == 0
        ...
        xlabel('time(ps)')
        ylabel('N_a_v_e')
        title('Average Carrier Density')
        hold off

        subplot(3,2,4)
        plot(z*10000, S_f, 'r'); hold on
        plot(z*10000, S_r, 'b');
        xlim(XL*1e4)
        % ylim([0, max([max(S_f), max(S_r), 1e10])])
        xlabel('z(\mum)')
        ylabel('S')
...
xlim([-f0*5e-15, f0*5e-15])  % Display ±5 GHz around the peak
xlabel('GHz')
ylabel('20log_1_0(|E|)')
legend('Output')
title('FFT')
...
plot(z*10000, kappa, 'b');
xlabel('z (\mum)')
ylabel('\kappa')
title('Grating Coupling Coefficient with \lambda_B/4 Phase Shift')