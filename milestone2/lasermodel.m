...
InputParasL.we = 1e13;       % rotation frequency
...
for i = 2:Nt                    % loop while time is between 2 and Nt (number of time units)
    ...
    Ef(2:Nz) = fsync*exp_det(1:Nz-1).*Ef(1:Nz-1);        % value of forward envelope past the first index
    Er(1:Nz-1) = fsync*exp_det(2:Nz).*Er(2:Nz);        % value of the reverse envelope previous to the final index
...
subplot(3,2,4)
plot(omega,real(fftInput),'b'); hold on
plot(omega,real(fftOutput),'g'); hold off
xlim([-mean(real(fftInput))/2,mean(real(fftInput))/2])
ylim([0,10e6])
xlabel('THz')
ylabel('|E|')
legend('Input','Output')
...
subplot(3,2,6)
plot(omega,unwrap(real(fftInput)),'b'); hold on
plot(omega,unwrap(real(fftOutput)),'g'); hold off
xlim([-1.5e13,1.5e13])
ylim([-10e6,10e6])
xlabel('THz')
ylabel('phase (E)')
